...
{
	float pivot = list[high].dtb;    
	int left = low;
	int right = high - 1;
	while (true) {
		while (left <= right && list[left].dtb < pivot) left++;
		while (right >= left && list[right].dtb > pivot) right--;
		if (left >= right) break;
...
void SapTheoHoTen(student list[], int low, int high)
{
	if (low < high)
	{
		int pi = partition(list, low, high);
		SapTheoHoTen(list, low, pi - 1);
		SapTheoHoTen(list, pi + 1, high);
	}
}
int main1()
{
